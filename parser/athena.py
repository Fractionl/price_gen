                    ...
                    for i in itertools.product(projections, widths, add_motor, add_opt1, add_opt2, add_opt3):
                        ...
                        if ((projection == 96 and width <= 132) or (projection == 120 and width >= 144) or (
                            ...
                            if (motor == 0 and projection <= 120 and width <= 216) or (motor == 1):
                                ...
                                opt1_price = 0.0
                                opt3_price = 0.0
                                for element in athena['sku']:
                                    if athena['sku'][element]['skucode'] == base_key:
                                        price = athena['sku'][element]['skuprice']
                                        motor_price = athena['sku'][element]['motor']
                                        opt1_price = athena['sku'][element]['hoodCover']
                                        opt3_price = athena['sku'][element]['dropShade']
                                        break
